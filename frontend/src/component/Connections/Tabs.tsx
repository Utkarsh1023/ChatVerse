      ...
      {TABS.map((tab) => {
        ...
        return (
          <motion.button
            ...
            className={`relative flex items-center gap-2 rounded-2xl px-4 py-2.5 text-xl font-medium transition ${
              isActive
                ? "text-white"
            ...
            {count > 0 && (
              <span
                className={`relative z-10 flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-xs ${
                  isActive ? "bg-white/20" : "bg-violet-500/20 text-violet-300"
                }`}
              >
                {count}
              </span>
